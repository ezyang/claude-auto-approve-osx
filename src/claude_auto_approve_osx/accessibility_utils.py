...
def dump_application_hierarchy(app_name, max_depth=3):
    ...
    def traverse(element, depth=0, path=""):
        ...
        title_error, title_value = AXUIElementCopyAttributeValue(
        ...
        for attr, value in all_attrs.items():
            if attr not in ["AXRole", "AXTitle", "AXChildren"] and not attr.startswith(
                "AX"
            ):
        ...
        if children_error:
            output.append(
                f"{'  ' * (depth + 1)}[Error getting children: {children_error}]"
            )
            return
